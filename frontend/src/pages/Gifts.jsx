...
const Gifts = () => {
  ...
  // Filtering cadeaux with gender filter and price sorting - UPDATED
  const filteredCadeaux = useMemo(() => {
    let filtered = products.filter((cadeau) => {
      const finalPrice = calculateFinalPrice(cadeau);

      const matchesGender =
        selectedGenders.length === 0 ||
        selectedGenders.includes("all") ||
        selectedGenders.includes(cadeau.gender);
  ...
  return (
    <div className="min-h-screen bg-background pt-20">
      <div className="costum-section">
        ...
        {!loading && (
          ...
          >
            <p className="text-primary/60 text-sm sm:text-base md:text-lg">
              {filteredCadeaux.length} coffret
              {filteredCadeaux.length > 1 ? "s" : ""} cadeau
              {filteredCadeaux.length > 1 ? "x" : ""} trouvé
              {filteredCadeaux.length > 1 ? "s" : ""}
              {selectedGenders.length > 0 &&
                !selectedGenders.includes("all") &&
        ...
        {/* Cadeaux Grid - Only show when not loading */}
        {!loading && (
          <motion.div
        ...
        {!loading && filteredCadeaux.length === 0 && (
          <motion.div
...
// Cadeau Card - UPDATED with discount display
const CadeauCard = ({ cadeau, formatPrice, calculateFinalPrice }) => {
  const { addToCart } = useCartStore();

  // حساب السعر النهائي
  const finalPrice = calculateFinalPrice(cadeau);
  const hasDiscount = cadeau.discount > 0;
