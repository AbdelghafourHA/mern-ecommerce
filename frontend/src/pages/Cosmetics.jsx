...
const Cosmetics = () => {
  ...
  useEffect(() => {
    getProductsByCategory("Cosmétiques", localSearch);
  }, [
    filters.gender,
    filters.maxPrice,
    filters.sort,
      ...
      <div className="costum-section">
        ...
        >
          <SearchPanel
            ...
            placeholder="Rechercher un cosmétique par nom ou marque..."
          />
        </motion.div>
        ...
        {!loading && (
          ...
          >
            <p className="text-primary/60 text-sm sm:text-base md:text-lg">
              {pagination.totalProducts} cosmétique
              {pagination.totalProducts > 1 ? "s" : ""} trouvé
              {pagination.totalProducts > 1 ? "s" : ""}
              {/* عرض مصطلح البحث إذا كان موجوداً */}
              {localSearch && (
                <>
                  {" "}
                  • Recherche:{" "}
        ...
        {!loading && products.length === 0 && (
          ...
          >
            <div className="text-6xl mb-4">{localSearch ? "🔍" : "💄"}</div>
            ...
            <p className="text-primary/80 mb-6 text-sm sm:text-base">
              {localSearch
                ? `Désolé, aucun cosmétique ne correspond à "${localSearch}"`
      ...
      <Link to={`/products/${cosmetic._id}`}>
        <div className="relative overflow-hidden aspect-square">
          ...
          <div className="absolute top-2 left-2 bg-accent text-primary px-2 py-1 rounded-full text-[10px] sm:text-xs font-semibold capitalize">
            {Array.isArray(cosmetic.gender) && cosmetic.gender.length > 1
              ? "Unisexe"
              : Array.isArray(cosmetic.gender) && cosmetic.gender.length === 1
              ? cosmetic.gender[0]
              : Array.isArray(cosmetic.gender)
              ? cosmetic.gender.join(", ")
              : cosmetic.gender}
          </div>
  ...
  return (
    ...
    >
      <div className="relative">
        ...
        <div className="hidden sm:flex mt-2 flex-wrap gap-2 text-xs text-primary/40">
